...
def _trace_ai(surface: str, prompt: str, completion: str | None, t0: float, *, ok: bool,
    ...
    # latency), the queryable + certification-embeddable record behind the card's "Generated by …"
    # line. Best-effort + lazy import so it never fails the AI call and never risks a circular.
    try:
        import core
        core.store.record_ai_call(surface=surface, provider="ollama", model=mdl,
                                  zone=provenance()["zone"], latency_ms=latency_ms, ok=ok,
