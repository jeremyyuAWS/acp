...
RULE_FORMATS: dict[str, frozenset[str]] = {
    ...
    # criterion, which this table cannot express but coverage=PARTIAL can. pdf 2.4.4 and xlsx 3.1.2
    # are now also ABSENT for the same reason — both are registry-backed with PARTIAL coverage.
    # `_ALL_FORMATS` minus {docx, pdf} for 2.4.4; `_ALL_FORMATS` minus {docx, xlsx} for 3.1.2.
...
REVIEW_FORMATS: dict[str, frozenset[str]] = {
    # 2.1.2 No Keyboard Trap: REMOVED — docx, pptx, and xlsx are all registry-backed now
    # (formats/docx, formats/pptx, formats/xlsx — each with PARTIAL coverage). The review-lane
    # branch runs BEFORE the registry branch and returns NOT_EVALUATED on a clean file, so leaving
    # a format here would silently shadow the coverage declaration and produce the wrong answer
    # for a file with no controls found.
    #
...
SCOPE_PRESETS: dict[str, dict[str, frozenset[str]]] = {
    ...
    # verdict for. 17 criteria, 62 pairs.
    #
    ...
    # pptx-only, 2.4.3 is pdf+pptx, 2.1.2 has no pdf lane. A flat 17 × 4 grid would claim ten
    # pairs the engine cannot judge.
    "acp-core-17": {
        ...
        "1.4.1": frozenset({"docx", "pdf", "pptx", "xlsx"}),
