...
|---|---|---|---|
| **Worker-tier heartbeat** | every scheduler tick (worker_main.py) | the standalone worker container, into the `worker_tier_heartbeat` setting | `GET /readyz` (`workers.heartbeat_at`, `workers.age_s`, `workers.alive`, `workers.pool_size`, `workers.version`), `GET /jobs` (`worker_tier_alive`), `POST /discovery/preflight` (`workers` block). `workers.version` is the worker image's `ACP_BUILD_VERSION` — the ONLY way to learn which image the worker tier is running, since `acp-worker` has no ingress; compare it against `GET /healthz`'s `version` to see whether a deploy reached both tiers, which nothing sequences (ADR 0045 §6). `null` means the beat predates the field, which is a different answer from `"dev"` (an image that never went through deploy.sh). |
            ...
            "HealthzResponse": {
                ...
                "properties": {
                    ...
                    "commit": {"type": "string", "nullable": True,
                               "example": "d914f31425fac1df0216a27b7442d0cbc105c320",
                               "description": "The commit this image was built from, so 'is commit X live?' is a read rather than an inference from build timestamps. Null on an image that cannot name one (built before this field, or by a bare docker build). A '-dirty' suffix means deploy.sh built from a working directory with uncommitted changes."},
