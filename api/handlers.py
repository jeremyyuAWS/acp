...
def _publish_file_guarded(payload: dict, job: dict) -> None:
    ...
    exception_release = bool(payload.get("allow_unverified") and payload.get("exception_manifest"))
    if not record or (not record.get("compliant") and not exception_release) or not record.get("remediated_at"):
        _release_failure(release_id, owner, filename, record or {}, "not_approved",
                         "Only approved corrected copies or acknowledged exception releases can be released.")
        return
    ...
    try:
        ...
        record = require_current_record(core.store, scan_id, filename, content_digest,
                                        ...
                                        allow_unverified=exception_release)
        ...
        core.store.record_release_document(release_id, owner, {
            ...
            "release_disposition": payload.get("release_disposition") or ("with_exceptions" if exception_release else "verified"),
            "exception_manifest": payload.get("exception_manifest"),
        })
