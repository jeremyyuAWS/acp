...
def holiday_enforcement(schedule: sched.Schedule) -> dict:
    """Describe holiday enforcement, which requires ACP reconciliation beyond KEDA cron."""
    holidays = tuple(schedule.holidays or ())
    return {
        "declared": list(holidays),
        "enforced_by_policy": True,
        "mechanism": "capacity_reconciler" if holidays else "weekly_cron",
        "reason": ("" if not holidays else
                   "ACP applies off-hours floors for the full local holiday and restores the "
                   "published weekly policy automatically afterward."),
    }
