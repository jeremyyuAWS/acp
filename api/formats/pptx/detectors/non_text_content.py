"""1.1.1 Non-text Content — pptx images.
...
WHY THIS EXISTS, when PPTX-ALT-001 already covers the pair in the partner catalog: the write-back
...
RULE_FORMATS has always listed pptx for 1.1.1 — the pass/fail lane was declared, and only the
...
so declaring them here is what puts pptx 1.1.1 in the index at all.
"""
...
def detect(path: Path) -> list[dict]:
    """One 1.1.1 finding per pptx image with no usable alt text."""
    return [{
        "ruleId": "PPTX_IMAGE_NO_ALT",
        "wcag": "1.1.1 Non-text Content",
        "severity": "CRITICAL",
        "detail": f"picture \u201c{img['name']}\u201d has no alt text \u2014 a screen reader announces nothing for it",
        "locator": img["locator"],
