"""1.1.1 Non-text Content — docx images.
...
WHY THIS EXISTS, when DOCX-ALT-001 already covers the pair in the partner catalog: the write-back
...
RULE_FORMATS has always listed docx for 1.1.1 — the pass/fail lane was declared, and only the
...
so declaring them here is what puts docx 1.1.1 in the index at all.
"""
...
def detect(path: Path) -> list[dict]:
    """One 1.1.1 finding per docx image with no usable alt text."""
    return [{
        "ruleId": "DOCX_IMAGE_NO_ALT",
