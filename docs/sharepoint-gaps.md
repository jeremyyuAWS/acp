...
| Archival rule: **user-based (departed employee)** | ✅ **Built 2026-09-05.** `owner` was already a match field, but there was **nowhere to put a roster**: match conditions are ANDed with no OR, so "owned by anyone on this list of 200 leavers" was 200 separate policies, each with its own approval and audit trail. This row previously described the gap as an input UTSW owed, which read as though a mechanism existed. The `in` operator is that mechanism — `{"field": "owner", "op": "in", "value": [...]}`, one rule for the whole roster, ANDed with an age condition as usual. | **Closed as a mechanism**; the roster itself is still UTSW's to supply, which is what the SOW says. Two behaviours worth knowing: `in` is **case-insensitive** (like `contains`/`prefix`, unlike `eq`) because every value it is written against is an identity exported from another system, and a casing mismatch would be a silent miss on exactly the files the rule is for; and a **multi-value** managed column intersects rather than failing. The rule builder does not offer it yet — a roster is pasted or POSTed, not typed one name at a time — so today it is an API-level rule. `not_in` ("archive anything owned by nobody on the CURRENT staff list") is the obvious sibling and was **not** added unasked; note that with AND-only conditions there is no way to express it otherwise. |
