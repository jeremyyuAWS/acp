...
| Archival rule: **user-based (departed employee)** | ✅ **Built 2026-09-05.** `owner` was already a match field, but there was **nowhere to put a roster**: match conditions are ANDed with no OR, so "owned by anyone on this list of 200 leavers" was 200 separate policies, each with its own approval and audit trail. This row previously described the gap as an input UTSW owed, which read as though a mechanism existed. The `in` operator is that mechanism — `{"field": "owner", "op": "in", "value": [...]}`, one rule for the whole roster, ANDed with an age condition as usual. | **Closed as a mechanism**; the roster itself is still UTSW's to supply, which is what the SOW says. Two behaviours worth knowing: `in` is **case-insensitive** (like `contains`/`prefix`, unlike `eq`) because every value it is written against is an identity exported from another system, and a casing mismatch would be a silent miss on exactly the files the rule is for; and a **multi-value** managed column intersects rather than failing. The rule builder does not offer either yet — a roster is pasted or POSTed, not typed one name at a time — so today they are API-level rules. **`not_in`** is the complement ("archive anything owned by nobody on the CURRENT staff list"), and with AND-only conditions there is no way to express that at all otherwise. It is **not** the boolean negation of `in`: an absent value matches NEITHER, so `in(x) or not_in(x)` is not always true. `not_in` selects documents FOR archival, and a document whose owner Graph refused to hand over has an owner — ACP just could not read it. Reading that silence as "owned by nobody on the staff list" would act on an absence as though it were a fact, in the destructive direction, and on an estate where the owner column was refused it would flag every document. That diverges from `ne`, which passes on an absent value; `ne` is unchanged, because every existing rule using it would move. |
| **Smart archival — check active collaborators before flagging** | ✅ **Built 2026-09-04.** `collaborator_count` is a lifecycle-rule field on every SharePoint row, at two precisions that it never travels without naming (`collaborator_basis`). **`authorship`** — the creator and the last editor, off the listing page's own base `$select`: free on every tier including the bare one, and a **floor** (a document twelve people edited names two of them). **`permissions`** — everyone with access, from the item's permissions collection: accurate, and one Graph call per document, so it is `ACP_SP_PERMISSIONS=1` and budgeted (`ACP_SP_PERMISSIONS_MAX`). Both cells are in the inventory export. | **Closed**, with the precision stated rather than assumed. A rule written `modified_age_days > 2555 AND collaborator_count <= 1` is correct under **either** basis — a floor of 1 means one person made it and nobody else ever touched it — which is why the free signal shipped rather than waiting on the switch. An operator who will not act on an approximation adds `collaborator_basis eq "permissions"` and the rule refuses the floor. Note what a count of 0 does **not** mean: an anonymous or organisation-wide sharing link grants access to people the permissions collection cannot name, so `sharing_scope` remains the field that says a file is shared. ✅ **Activity added 2026-09-05**, because access is not use: `collaborator_count` says who CAN open a document, and `recent_actor_count` / `recent_action_count` say whether anybody HAS, over Graph's own seven-day `/analytics/lastSevenDays` window (a fixed endpoint, not a window ACP chose). `ACP_SP_ANALYTICS=1`, budgeted by `ACP_SP_ANALYTICS_MAX`, off by default — one Graph call per document. The counts are `unavailable`, never `0`, on any document that was not measured: analytics is not served for every item or tenant, and a rule reading "nobody opened this in seven days" off an unanswered question would archive a live estate. So the full smart-archival rule is `modified_age_days > 2555 AND collaborator_count <= 1 AND recent_actor_count eq 0`, and on an unmeasured estate it matches nothing rather than everything. |
...
| **File write-back preconditions** (the remediated-document replace path, dormant until a write scope exists) | ✅ **Built 2026-09-05.** `sharepoint_upload`'s in-place replace archived the original and THEN replaced it, with nothing between asking whether the replace could succeed — so a checked-out file failed *after* a dated copy had landed in `SP_ARCHIVE_FOLDER`, indistinguishable from a real archive in the folder a customer would use to roll a remediation back. `api/sp_writeback.py` now reads the item's live state first and refuses with `409` on a **check-out** (the write would fail anyway) or a **declared record** (`_IsRecord` — a governance decision, not a remediation). `sp_metadata` had read both since Phase 2 and its own docstring called check-out "the precondition Phase 5 has to check"; nothing consumed it. | **Closed**, and the boundaries matter as much as the blocks: a retention **label** alone does not block (it commonly sets a period without locking edits, so refusing would turn away writes that would have succeeded), and an **unreadable** precondition does not block either — it proceeds and the response says `preconditionsChecked: false`, because "we did not look" must not read as "we looked and it was fine". One Graph call per write-back, read live rather than from the scan: a file checked out *since* the scan is the case this catches, and a write-back is per-file and approval-gated, so the call is proportionate here where the same call in a 30-site listing is an outage. |
...
- **`ACP_SP_PERMISSIONS`** (default `0`) — read each item's permissions collection, which turns
  `collaborator_count` from the free authorship floor into everyone with access. Off because it
  ...
  with the switch named in the reason, and `collaborator_basis` reads `authorship`.
  **It did nothing at all until 2026-09-04** — `sp_metadata.permissions_enabled()` shipped in
  Phase 2 with no caller, so the switch the field's own reason string told operators to set was
  inert. A documented switch that does nothing is worse than an undocumented gap: it ends the
  search for why the field is empty.
- **`ACP_SP_ANALYTICS`** (default `0`) / **`ACP_SP_ANALYTICS_MAX`** (default `1000`) — read each
  item's activity over Graph's seven-day window (`/analytics/lastSevenDays`), which is what
  distinguishes a document nobody has opened since 2019 from one somebody reads every week. Off
  and budgeted for the same reason as the two knobs around it: one Graph call per document. A
  document that was not measured reports `recent_actor_count` **unavailable, never 0** — analytics
  is not served for every item or tenant, and an unanswered question that reads as "idle" is the
  one that archives a live document.
- **`ACP_SP_PERMISSIONS_MAX`** (default `1000`) — how many documents one listing may read
  permissions for once the switch above is on. Same reason and same shape as
  `ACP_SP_CONTENT_TYPE_MAX`: turning a per-document read on must not turn it on unbounded.
  What the cap costs is **precision, not the field** — a document past it keeps the authorship
  floor, a smaller number that is labelled as one, so a `collaborator_count <= 1` rule still
  evaluates on it rather than silently skipping it.
