...
| Archival rule: **user-based (departed employee)** | ✅ **Built 2026-09-05.** `owner` was already a match field, but there was **nowhere to put a roster**: match conditions are ANDed with no OR, so "owned by anyone on this list of 200 leavers" was 200 separate policies, each with its own approval and audit trail. The `in` operator is that mechanism — `{"field": "owner", "op": "in", "value": [...]}`, one rule for the whole roster, ANDed with an age condition as usual. The lifecycle rule builder accepts a pasted newline/comma/semicolon-separated roster for both `in` and `not_in`, deduplicating identities case-insensitively before preview/save. | **Closed as a mechanism and UI workflow**; the roster itself is still UTSW's to supply, which is what the SOW says. `in` is case-insensitive because identities exported from another system vary in casing. **`not_in`** is the complement ("archive anything owned by nobody on the CURRENT staff list"). It is deliberately not the boolean negation of `in`: an absent/unread owner matches neither, so missing metadata is never treated as evidence for archival. |
