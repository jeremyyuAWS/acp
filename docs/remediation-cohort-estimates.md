...
A matching calibration record supplies format, change family, exact configuration ID, immutable evaluation version, evaluated date, and evaluated (never synthetic) dataset/report provenance with representative and production-approved flags plus an operator qualification reference. Hashes alone do not establish representative production evidence. At least 30 independent labelled finding cases within 30 days and before expiry are required. The default is conservative availability, not an administrator policy floor or application decision.
...
`estimate_plan` accepts only a **server-produced** complete population with immutable scope and assessment revisions, a configuration revision equal to the registry-computed configuration digest, and exact eligible finding/source identities, format, family, and configuration. Mixed configurations/populations are unavailable pending stratified support. It selects the newest matching evaluation without searching older records for a better rate. The reader additionally requires a trusted current `estimate_context` with matching revisions, rereads the sealed assessment output manifest, and recomputes the scope digest from selected preview files. Absent current configuration context, or a changed manifest/scope, suppresses estimates. No current authoritative family/configuration producer exists; registry records alone never stand in for live configuration.

The read-only `POST /scans/{sid}/remediation/impact-estimate` reuses the existing routing preview's owner, scope and policy checks and no-store response. Plan preview embeds the same `estimated_impact` result. Neither route accepts evaluation evidence or caller-supplied eligible finding counts.
...
Tests cover complete, sparse, stale, synthetic, mixed and out-of-population evidence; historical missing lineage; deduplicated outcomes and charge replays; held/unknown/missing charges; private content stripping; and stale UI scope refreshes, superseded deferred responses, and same-email logout/session replacement. The caller binds responses to API base/authentication epoch, owner, run, selected scope, policy, and refresh identity.

## Explicit generation-chain qualification

An explicit generation chain is part of evaluation configuration identity: its version and all ordered step IDs, positions, providers, models, enabled states and capabilities are normalized using the same execution validator. The primary step must agree with the declared generator. Reviewer and validator identities remain in the digest. Two- and three-position chains, changed fallback models, reordered models and changed providers cannot share qualification.

Historical records without a chain retain their original configuration IDs for sealed-policy compatibility. Absence is never treated as a default explicit two-position chain. Such records cannot qualify an explicit two- or three-position request; qualified evidence requires a new immutable evaluation version recording that exact chain. No stored evaluation is rewritten.

The estimate reader compares selected policy, current context and population chains before reading cohorts. The population chain then accompanies applicability into cohort comparison, independently of the configuration digest, so a mistakenly reused old digest cannot yield old impact or cost ranges. The Plan response key already includes the whole policy, including chain changes; no new UI cache is introduced.
