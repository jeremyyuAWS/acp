...
3. Download the source again, the delivered copy, both effective permission snapshots, and ACP's
   server-generated release manifest.
...
source and delivered provider IDs differ; the release contains exactly one successful document;
and the delivered permissions match the approved expectation. Retain the bundle and report with the
