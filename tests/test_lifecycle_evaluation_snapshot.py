...
def test_immutable_evidence_summary_and_file_detail(isolated_store, monkeypatch):
    ...
    assert summary["integrity"]["evidence_complete"] is True
    assert summary["integrity"]["candidate_count"] == 1
    assert summary["integrity"]["candidates_with_evidence"] == 1
...
def test_candidate_status_without_evidence_fails_integrity_and_is_counted(isolated_store):
    st = isolated_store
    with st._db.cursor() as cur:
        st._db.execute(cur, "INSERT INTO scan_runs(id,owner_email,status,source,scope) VALUES(%s,%s,%s,%s,%s)",
                       ("broken", "owner@x.com", "discovered", "drive",
                        json.dumps({"lifecycle_rules_enabled": 1})))
    st.add_inventory("broken", [{"file": "old.docx"}])
    st.set_lifecycle_status("broken", "old.docx", "Archive Candidate", rule_id="retention")
    summary = st.lifecycle_summary("broken", "owner@x.com")
    assert summary["integrity"] == {
        "evidence_complete": False, "expected_rules": 1, "recorded_rules": 0,
        "evaluations": 0, "evaluated_files": 0, "candidate_count": 1,
        "candidates_with_evidence": 0,
    }
    assert st.count_lifecycle_files("broken", "owner@x.com", candidate_only=True) == 1
    assert [r["file"] for r in st.list_lifecycle_files(
        "broken", "owner@x.com", candidate_only=True)] == ["old.docx"]
