...
spec = importlib.util.spec_from_file_location('configure_openai', Path(__file__).parents[1] / 'scripts/configure_openai_secret.py')
...
def test_secret_only_flows_to_secret_store(monkeypatch, capsys):
    ...
    assert all(args[-1] == 'OPENAI_API_KEY=secretref:openai-api-key' for args in calls[4:])
