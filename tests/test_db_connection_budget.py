...
def test_rightsize_script_sets_worker_pool_and_queue_autoscale():
    ...
    # THAT there is a queue predicate, not WHICH types it names. This assertion used to pin the
    # literal list, which made it the fifth copy of core.REMEDIATE_LANE_JOB_TYPES in the tree and
    # the second one to go red for a job type added to the first. What the list must contain is
    # asserted once, derived from core, in test_packaging_chart.py::
    # test_the_production_autoscaler_counts_every_remediate_job_type — so a new handler is a
    # one-place change and this file keeps testing the script's SHAPE, which is what it is about.
