...
| `postgres` | `max_connections` | >= 115 | derived | the fleet's worst case at maximum replicas is 100 (acpctl.inventory.connection_budget, which mirrors api/store.py's db_max_conn per replica), plus 15 the server keeps for itself |
