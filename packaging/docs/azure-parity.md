...
**Production does not autoscale the assess and remediate tiers at all.** Both are pinned at five
replicas — `min == max` — and `rightsize-production.sh` says why: *"Assessment and Remediation are
...
The contract's standard-production example describes those same tiers as autoscaling **3–10** on
queue depth, and the Helm chart renders KEDA `ScaledObject`s to make that happen. So adopting the
chart on Azure as it stands would not be a like-for-like rebuild: it would **replace a warm pool
with an autoscaler**, on the two stages whose latency somebody deliberately traded capacity for.
...
matches. The replica ranges do not, and only one of the four differences (the API floor) is
...
| Container app | Tier | CPU | Memory | Replicas | Scales? | Ingress | Scale rules | Read from |
|---|---|---:|---:|---|---|---|---|---|
| `acp-app` | `api` | 1.0 | 2Gi | 1–3 | yes | external | none in this repo | rightsize-production.sh + deploy.sh |
| `acp-assess` | `assess` | 2.0 | 4Gi | 5–5 | **no** | none | none in this repo | rightsize-production.sh |
| `acp-discovery` | `discover` | 1.0 | 2Gi | 1–2 | yes | none | none in this repo | rightsize-production.sh |
...
| `acp-remediate` | `remediate` | 2.0 | 4Gi | 5–5 | **no** | none | none in this repo | rightsize-production.sh |
...
| `remediate` | `replicas.max` | `5` | `10` | **unexplained** | — |
| `remediate` | `autoscaled` | `False` | `True` | **unexplained** | — |
