...
describe('v2: scan scope is a pre-discovery step', () => {
  ...
  it('is byte-identical to the frontend/ copy it was ported from', () => {
    // Two SPAs, one component, until v2 deliberately redesigns it. Any divergence should be a
    // decision someone made, not a patch that landed in one tree and not the other.
    const a = read('ScanScope.jsx')
    const b = readFileSync(join(HERE, '..', '..', 'frontend', 'src', 'ScanScope.jsx'), 'utf8')
    expect(a).toBe(b)
  })
})
