...
const PHASES = [
  // ── Google Drive ────────────────────────────────────────────────────────────
  {
    ...
    items: [
      { text: 'Google Cloud project + Drive API v3 enabled', status: 'done' },
      { text: 'OAuth 2.0 web client (Google Identity Services sign-in button)', status: 'done' },
      { text: 'Drive file listing by MIME type (docx, pdf, pptx, xlsx, html)', status: 'done' },
      { text: 'File metadata sync: name, owner, modified date, size, shared status', status: 'done' },
      { text: 'Token refresh + session management (sessionStorage token)', status: 'partial' },
      { text: 'Pagination for large Drive folders (pageToken handling)', status: 'done' },
  ...
  },
  {
    ...
    items: [
      { text: 'Drive file download (API v3 /files/{id}?alt=media)', status: 'done' },
      { text: 'Export Google Docs → DOCX via Drive export endpoint', status: 'done' },
      { text: 'Export Google Sheets → XLSX, Slides → PPTX', status: 'done' },
  ...
  },
  {
    ...
    items: [
      { text: 'Upload remediated file to Drive (Files.update — same ID, new content)', status: 'done' },
      ...
      { text: 'Permission-aware upload: check write scope before attempting (handle read-only shares)', status: 'partial' },
