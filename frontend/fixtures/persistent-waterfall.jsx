...
function Fixture() {
  ...
  const snapshot = { scan_id: 'saved-scan', run_id: 'saved-scan', batch_id: `${run}-batch`, state,
  ...
  const activity = { view: { available: run !== 'legacy', ai_enabled: true, generated_at: snapshot.generated_at,
    stages: run === 'legacy' ? [] : [{ tier: 1, operations: 3, active: state === 'running' ? 1 : 0, settled: 2, models: [{ provider: 'recorded-provider', model: 'recorded-model-v1' }] }, ...(run === 'multiple' ? [{ tier: 2, operations: 5, models: [{ provider: 'recorded-provider', model: 'fallback-model-one', recorded_attempts: 2 }, { provider: 'second-provider', model: 'fallback-model-two', recorded_attempts: 3 }] }] : [])] } }
  return <main style={{ maxWidth: 1200, padding: 16, margin: 'auto' }}>
    ...
    <label>Saved run <select aria-label="Saved run" value={run} onChange={e => setRun(e.target.value)}><option value="saved">Saved run</option><option value="legacy">Legacy run</option><option value="multiple">Multiple recorded models</option></select></label>
