...
it('uses one scan action and distinguishes similar findings from ready fixes', async () => {
 ...
 expect(v.button('Apply ready fixes (1)')).toBeTruthy()
 ...
 await click(v.button('Apply ready fixes (1)'))
...
it('freezes a whole-scan action and never adds later proposals or duplicates busy writes', async () => {
 ...
 await click(v.button('Apply ready fixes (2)'))
 expect(onDecide).toHaveBeenCalledTimes(1)
 expect(v.button('Apply ready fixes (2)').disabled).toBe(true)
