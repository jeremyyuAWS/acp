...
it('shows only two questions and an honest disabled budget when AI is enabled', async () => {
  ...
  await render(1)
  expect([...container.querySelectorAll('legend')].map(n => n.textContent)).toEqual(['1. How much automation?', '2. Use AI?'])
  ...
  expect(container.textContent).toContain('Spending limits are not enforced yet')
