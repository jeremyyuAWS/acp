...
it.each(['plan', 'modes'])('suppresses the accepted plan popup for legacy %s URLs and history', async mode => {
  history.replaceState({}, '', `/?tab=remediate&mode=${mode}`)
  const { root, container } = await mount({ planAccepted: true })
  expect([...container.querySelectorAll('button')].some(node => node.textContent === 'Remediation plan')).toBe(false)
  expect(container.querySelector('dialog').open).toBe(false)
  await act(async () => window.dispatchEvent(new PopStateEvent('popstate')))
  expect(container.querySelector('dialog').open).toBe(false)
  await act(async () => root.render(createElement(RemediationWorkspaceTabs, { ...props, planAccepted: true, workspaceRequest: { mode: 'plan' } })))
  expect(container.querySelector('dialog').open).toBe(false)
  expect(container.querySelector('#rem-panel-live').hidden).toBe(false)
})
