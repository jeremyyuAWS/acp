...
it('allows an explicit source-check retry after reconnect when no repair request was sent', async () => {
 vi.useFakeTimers()
 const repair=vi.fn().mockRejectedValueOnce(Object.assign(new Error('Microsoft connection required'),{code:'microsoft_connection_required',sourceIdentityRequestSent:false})).mockResolvedValue({status:'repaired'})
 const read=vi.fn().mockResolvedValue({planning:repairablePlanning})
 const v=await mount({read,repair,requireChoice:true})
 expect(v.container.textContent).toContain('Reconnect Microsoft in Sources')
 await act(async()=>vi.advanceTimersByTimeAsync(5000))
 expect(repair).toHaveBeenCalledTimes(1)
 read.mockResolvedValueOnce({planning:repairablePlanning}).mockResolvedValue({planning})
 const retry=[...v.container.querySelectorAll('button')].find(b=>b.textContent==='Retry source checks after reconnecting')
