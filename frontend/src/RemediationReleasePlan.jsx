...
export default function RemediationReleasePlan({ scanId, files, intent, onChange, disabled = false, read = getAutomaticRelease, repair = (...args) => repairAutomaticReleaseSourceIdentity(...args), requireChoice = false, compact = false, onAnswered }) {
  ...
  return <section className="rem-auto-release" aria-label="Release option for this plan">
    ...
    {ready && <p role="status"><strong>Automatic publishing covers {preview.files.length} of {files.length} selected files.</strong>
      {preview.files.length < files.length && <> {files.length - preview.files.length} files are excluded by assessment or source checks. They stay in the follow-up list; becoming ready later does not add them to this saved permission.</>}
    </p>}
