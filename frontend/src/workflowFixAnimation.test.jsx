...
it('shows green +fix feedback in the visible workflow card and clears it without replaying another run', async () => {
  ...
  expect(container.querySelector('.finding-outcome-kpis__verified .kpi-update-delta--positive').textContent).toBe('+2')
  expect(container.querySelector('.tone-amber .kpi-update-delta--positive').textContent).toBe('−2')
