...
const proposal={id:'proposal',file:'a.docx',status:'pending',hasProposal:true,after:'Alternative text',proposals:[{proposed_value:'Alternative text',source:'AI',model:'vision',model_call_id:'call'}],_raw:{finding_count:1,proposal_snapshot_ids:['snapshot'],source_revision:'source',decision_version:0}}
const admitted=state=>({...proposal,_raw:{...proposal._raw,automatic_approval:{run_id:'run',source_revision:'source',state}}})
...
const action=container=>[...container.querySelectorAll('button')].find(button=>['Apply this fix','Queued automatically','Applying automatically'].includes(button.textContent))
it.each([['queued','Queued automatically'],['checking','Queued automatically'],['processing','Applying automatically']])('disables duplicate manual approval for server-admitted %s work',async(state,label)=>{
...
it('does not disable approval merely because the switch is on',async()=>{
 ...
 expect(button.textContent).toBe('Apply this fix');expect(button.disabled).toBe(false)
...
it.each([{enabled:false},{supported:false},{run_id:'other'},{source_revision:'changed'}])('keeps unadmitted proposals actionable with mismatched policy %j',async change=>{
 ...
 expect(action(container).textContent).toBe('Apply this fix');expect(action(container).disabled).toBe(false)
})
