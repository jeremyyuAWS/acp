...
export default function ReleaseFileSelection({ files, selectedFiles, setSelectedFiles, done, sourceState, sourceProduct, releaseProvider, driveMirrorEnabled, driveMirrorFolder, releaseFolder, releaseResults, selectedFile: requestedFile, setSelectedFile, sourcePath, pending = {}, processing = {}, blockers = {}, allowRemainingIssues = false, destinationLabel }) {
  ...
  const rows = useMemo(() => files.map((file) => {
    ...
    return { file, folder: parts.length > 1 ? parts.slice(0, -1).join('/') : 'Source root', ...releaseReadiness(file, { done, results: releaseResults, sourceState, pending, processing, blockers, allowRemainingIssues }) }
