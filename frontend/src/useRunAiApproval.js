...
export default function useRunAiApproval(scanId, runId) {
  ...
  return { notice: notice?.identity === identity ? notice : null, dismissNotice: () => setNotice(null), policy: current, enabled: current?.enabled ?? null, saving, error: error || (current?.supported === false ? current.reason : ''), change }
}
