...
export default function RemediationWaterfallCard({ snapshot, paused = false, activity = null }) {
  ...
  const selectedStage = stages.find(stage => stage.tier === (selectedRole === 'first' ? 1 : selectedRole === 'next' ? 2 : null))
  const descriptions = {
  ...
  return <section className={`wf-card${visualsPaused ? ' wf-paused' : ''}`} aria-label="Live remediation waterfall">
    ...
    {stageDrawer && selectionScope === identity && createPortal(<Drawer title="Stage evidence and costs" onClose={closeStageDrawer}><div className="wf-detail">{selectedModel?.model && <h3>{selectedModel.provider} · {selectedModel.model}</h3>}<p>{descriptions[selectedRole]}</p>{selectedModel?.model && selectedStage?.models?.length > 1 && <p>Selected recorded model. Counts and history below cover its tier and the whole run; model sequence is unavailable.</p>}{selectedStage?.models?.map(model => <p key={`${model.provider}:${model.model}`}><strong>{model.provider} · {model.model}</strong></p>)}</div><RemediationAttemptStory scanId={scanId} batchId={batchId} modelFilter={selectedModel?.model ? selectedModel : null} defaultOpen={true} live={storyLive} paused={paused || motion.hidden} reviewHref={`${reviewUrl.pathname}${reviewUrl.search}${reviewUrl.hash}`} /><details className="wf-detail"><summary>Stage evidence and costs</summary>{stageEvidence}</details><RemediationRunInsights scanId={scanId} batchId={batchId} /></Drawer>, document.body)}
