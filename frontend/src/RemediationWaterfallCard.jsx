...
export default function RemediationWaterfallCard({ snapshot, paused = false, activity = null }) {
  ...
  return <section className={`wf-card${paused ? ' wf-paused' : ''}`} aria-label="Live remediation waterfall">
    ...
    <div className="wf-layout"><div><h4>Live AI waterfall</h4><ol className="wf-stages">
      ...
      {[1, 2].map(tier => {
        ...
        return <Stage key={tier} title={tier === 1 ? '02 · First AI' : '03 · Next AI'} detail={data?.available ? data.ai_enabled ? 'Bounded text drafting' : 'AI disabled for this run' : 'Attempt history unavailable'} identity={identity} stamp={JSON.stringify(stage)} paused={paused || state.error} selected={selection === selected} onClick={() => setSelection(selected)}>
          {stage ? <><span className="wf-stage-total">{displayCount(stage.operations)} recorded operations</span><span className="wf-secondary">{stage.active} dispatched · {stage.settled} settled · {stage.uncertain} uncertain</span></> : <span className="wf-secondary">No measured count available</span>}
        </Stage>
      })}
    ...
    </ol></div><aside className="wf-detail"><h4>{({ rules: 'Rules lead the way', first: 'What the first AI did', next: 'What the next AI did', approval: 'Your decision matters', verify: 'Evidence of completion' })[selection]}</h4><p>{descriptions[selection]}</p>
      ...
      <section><h4>Provider and model used</h4><p>Historical provider and model identities are unavailable for these ledger records.</p></section>
      <section><h4>Spending for this run</h4><dl className="wf-spending">{[['spent_units', 'Settled provider charges'], ['held_units', 'Reserved · may still be charged'], ['available_units', 'Remaining allowance'], ['cap_units', 'Approved spending limit']].map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{money(spending?.[key])}</dd></div>)}</dl>{spending?.unknown_charges > 0 && <p className="wf-note">{spending.unknown_charges} charge(s) unknown. Their reservations remain held.</p>}{spending?.blocked && <p className="wf-note">Further AI spending is blocked pending reconciliation.</p>}<p className="wf-secondary">Provider charges only. Infrastructure costs are separate.</p></section>
    </aside></div>
    <footer className="wf-footer"><span>{state.error ? 'Refresh delayed · showing the last recorded AI activity' : paused ? 'Visual updates paused' : 'Updates follow recorded activity'}</span><span>{data?.generated_at ? `AI snapshot ${new Date(data.generated_at).toLocaleTimeString()}` : 'Waiting for AI activity records'}</span></footer>
