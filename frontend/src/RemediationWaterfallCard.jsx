...
export default function RemediationWaterfallCard({ snapshot, paused = false, activity = null }) {
  ...
  return <section className={`wf-card${paused ? ' wf-paused' : ''}`} aria-label="Live remediation waterfall">
    ...
    <div className="wf-layout"><div><h4>Live AI waterfall</h4><ol className="wf-stages">
      ...
      <section><h4>Provider and model used</h4>{data?.models?.length ? <><ul className="wf-models">{data.models.map(model => <li key={`${model.provider}:${model.model}`}><strong>{model.provider} · {model.model}</strong><span>{model.linked_calls} recorded call{model.linked_calls === 1 ? '' : 's'} linked to current proposals</span><span>Recorded call cost: {typeof model.recorded_cost_usd === 'number' ? money(Math.round(model.recorded_cost_usd * 1000000)) : 'Unavailable'}</span></li>)}</ul><p className="wf-secondary">Identities come from saved call records. First-model versus fallback attribution is unavailable. These call costs are not additional to the run charges below.</p></> : <p>No provider/model identity is linked to this run’s proposals yet.</p>}</section>
