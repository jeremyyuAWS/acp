  ...
  return <section className="remediation-file-page" aria-label="Remediation details by WCAG success criterion">
    ...
    {Object.entries(groups).map(([sc, group]) => {
      ...
      return <article className="remediation-sc-card" key={sc}>
        ...
        {group.findings.map((finding, index) => <div className="remediation-sc-row" key={`finding-${index}`}><span>{finding.detail || 'Finding recorded'}</span><RemediationCategoryPill category={remediationCategory(finding)} /></div>)}
        {group.changes.map((change, index) => <div className="remediation-sc-change" key={change.id || index}>
          <h4>Change {index + 1}{change.page != null ? ` · Page ${change.page}` : ''} <RemediationCategoryPill category={change.verified === true ? 'verified' : 'applied'} /></h4>
          <dl><dt>Before</dt><dd>{text(change.before)}</dd><dt>After</dt><dd>{text(change.after ?? change.value ?? change.approved_value)}</dd></dl>
