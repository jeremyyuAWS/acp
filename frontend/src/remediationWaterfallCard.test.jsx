...
it('shows durable units, costs, honest missing contribution, and accessible outcomes', async () => {
  ...
  expect(document.querySelector('[role=dialog]').textContent).toContain('$1.18')
  expect(document.querySelector('[role=dialog]').textContent).toContain('$3.62')
  expect(document.querySelector('[role=dialog]').textContent).toContain('AI step breakdown unavailable')
  expect(container.querySelector('.wf-outcomes')).toBeNull()
  expect(container.textContent).not.toContain('Where your findings stand')
...
it.each([true, false])('keeps the requested outcome section retired, exact=%s', async exact => {
  const { root, container } = createTestRoot()
  await act(async () => root.render(<RemediationWaterfallCard snapshot={snapshot({ finding_reconciliation: { ...snapshot().finding_reconciliation, exact } })} />))
  expect(container.querySelector('.wf-outcomes')).toBeNull()
  expect(container.textContent).not.toContain('Where your findings stand')
  expect(container.textContent).not.toContain('Finding outcome totals unavailable')
  expect(container.textContent).not.toContain('Inspect outcomes')
  expect(container.querySelector('.wf-activity-action button')).not.toBeNull()
})
...
it('shows the recorded provider and exact model and preserves unknown costs', async () => {
  ...
  expect(document.querySelector('[role=dialog]').textContent).toContain('not a model breakdown for the selected run')
})
