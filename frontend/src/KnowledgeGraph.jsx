...
const CRIT = {
  ...
  SC_3_1_1: ['3.1.1', 'language of page'], SC_3_1_2: ['3.1.2', 'language of parts'],
...
export default function KnowledgeGraph({ files }) {
  ...
  useEffect(() => {
    ...
    const links = []
    files.forEach((f) => [...new Set(f.issues.map((i) => i.wcag))].forEach((c) => links.push({ source: f.file, target: c })))
  ...
  return (
    <div className="panel">
      ...
      <div className="kgdetail">
        {!detail && <span className="muted">Click a node — files cluster around the criteria they share. Drag to rearrange.</span>}
        {detail?.kind === 'file' && (
