...
function Progress({ snapshot, suspect }) {
  ...
  return <section className="remops-progress" aria-labelledby="remops-progress-title">
    ...
    {known && <div className="remops-segments" aria-label={`${total} documents: ${rows.map((r) => `${r.value} ${r.label.toLowerCase()}`).join(', ')}`}>{rows.filter((r) => r.value > 0).map((row) => <span key={row.key} className={`remops-segment remops-segment-${row.key}`} style={{ width: `${row.value / total * 100}%` }} title={`${row.label}: ${row.value}`} />)}</div>}
...
function Pipeline({ phases = [] }) {
  ...
  return <section className="remops-pipeline"><h3>Active document pipeline</h3><ol aria-label="Remediation phases">{phases.map((phase, index) => <li key={phase.key} className={`remops-phase remops-phase-${phase.status}`}><span className="remops-phase-mark" aria-hidden="true">{phase.status === 'active' ? '●' : phase.status === 'failed' ? '×' : phase.status.startsWith('completed') ? '✓' : '○'}</span><span className="remops-phase-name">{phase.label}</span><span className="remops-phase-state">{PHASE_TEXT[phase.status] || phase.status}{phase.detail ? ` · ${phase.detail}` : ''}</span>{index < phases.length - 1 && <span className="remops-flow" aria-hidden="true">···►</span>}</li>)}</ol></section>
}
...
function Workstream({ attempts = [], generatedAt }) {
  ...
  return <section className="remops-work"><h3>In flight now <span>· {attempts.length} document{attempts.length === 1 ? '' : 's'}</span></h3><ul>{shown.map((a) => { const signal = now && a.progress_at ? (now - Date.parse(a.progress_at)) / 1000 : null; return <li key={`${a.file}-${a.started_at || ''}`}><div className="remops-doc-head"><strong><span aria-hidden="true">●</span> <span className="fname">{a.file}</span></strong><span>{ago(a.elapsed_s) ? `in flight ${ago(a.elapsed_s)}` : ''}</span></div><div className="remops-trail"><span className="remops-done">✓ Opened</span><span aria-hidden="true">→</span><span className="remops-active">● {a.phase || 'Processing'}</span>{a.attempt > 1 && <span>attempt {a.attempt}</span>}{ago(signal) && <span>last signal {ago(signal)} ago</span>}</div></li> })}</ul>{attempts.length > 3 && <p className="muted">and {attempts.length - 3} more document{attempts.length - 3 === 1 ? '' : 's'} in flight</p>}</section>
}

function Throughput({ snapshot }) {
...
export default function RemediationOpsPanel({ snapshot = null, connected = false, receivedAt = null, events = [], updateMode = 'idle', onViewMonitor = null }) {
  ...
  return <section className={`panel remops${paused || hidden ? ' remops-motion-paused' : ''}`} aria-label="Remediation run status">
    ...
    <Progress snapshot={snapshot} suspect={suspect} /><Pipeline phases={snapshot.phases} />
    <div className="remops-two"><Workstream attempts={snapshot.active_attempts || []} generatedAt={snapshot.generated_at} /><Throughput snapshot={snapshot} /></div>
