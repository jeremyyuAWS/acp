...
export default function RemediationAutomationStatus({ policy, error, saving, reviewCount = 0, onOpenReview, onRetry }) {
  ...
  return <section className="remediation-automation-status" aria-label="Remediation automation">
    <div><span className="remediation-automation-status__label">AI fixes</span><h2>{label}</h2>
      ...
      {unavailable && <><p role="status">{error || policy.reason}</p>{onRetry && <button className="ghost small" onClick={onRetry}>Refresh setting</button>}</>}
    </div>
    <div><span className="remediation-automation-status__label">Review workspace</span><strong>{reviewCount.toLocaleString()}</strong><p>Review items still unresolved. This is an item count, not a count of findings or verified fixes.</p>
