...
it('publishes only ready files without opening details or reviewing others', async () => {
  const v = await mount(); await click(v.button('Publish batch (1)'))
...
it('keeps ready-only publishing available while the continuation runs', async () => {
  ...
  expect(v.button('Publish batch').disabled).toBe(false)
  expect(v.button('Approve eligible changes').disabled).toBe(true)
...
it('prevents writes in read-only replay', async () => {
  const v = await mount({ readOnly: true }); expect(v.button('Publish batch').disabled).toBe(true)
...
it('shows loading separately from known zero eligibility and preserves ready-only publishing', async () => {
  ...
  expect(v.button('Publish batch').disabled).toBe(false)
  await click(v.button('Publish batch'))
  expect(v.props.onReady).toHaveBeenCalledWith(['ready.pdf'])
...
it('keeps publishing primary and optional approval collapsed, with linked reasons', async () => {
  ...
  for (const label of ['Publish batch (0)', 'Approve eligible changes']) {
...
it('keeps both unavailable actions visible during loading and empty scope', async () => {
  ...
  expect(v.button('Publish batch').disabled).toBe(true)
  expect(v.button('Approve eligible changes').disabled).toBe(true)
...
it('shows three ordered steps and publishes only checked ready copies', async () => {
  ...
  expect([...v.container.querySelectorAll('h4')].map(h => h.textContent)).toEqual(['1 Publication batch', '2 Confirm destination', '3 Publish copies'])
  ...
  expect(v.button('Publish batch (0)').disabled).toBe(true)
...
it('reconnects Drive and resumes the saved manual authorization without approving again', async () => {
 ...
 it('shows ready copies without the retired requirements section', async () => {
  ...
  const publish = v.button('Publish batch')
...
it('defaults to one unpublished eligible batch while specific-file selection stays collapsed',async()=>{
 const records=['one.pdf','two.pdf','already.pdf','unwritten.pdf'].map(file=>({file}))
 const v=await mount({files:records,ready:records.slice(0,3),fileStates:{'already.pdf':{status:'released'},'unwritten.pdf':{status:'applying'}}})
 const chooser=v.container.querySelector('.release-quick-specific-files')
 expect(chooser.open).toBe(false)
 expect(chooser.querySelector('summary').textContent).toBe('Choose specific files')
 expect(v.container.textContent).toContain('All unpublished eligible copies are included in one batch')
 await click(v.button('Publish batch (2)'))
 expect(v.props.onReady).toHaveBeenCalledWith(['one.pdf','two.pdf'])
 expect(v.container.querySelector('[aria-label="Publish already.pdf"]')).toBeNull()
})
it('new eligible copies join the default batch and intentional exclusions can be reset',async()=>{
 const records=['one.pdf','two.pdf'].map(file=>({file}))
 const v=await mount({files:records,ready:records.slice(0,1)})
 await v.render({ready:records})
 expect(v.button('Publish batch (2)')).toBeTruthy()
 await click(v.container.querySelector('[aria-label="Publish one.pdf"]'))
 await click(v.button('Publish batch (1)'))
 expect(v.props.onReady).toHaveBeenLastCalledWith(['two.pdf'])
 await click(v.button('Include all eligible files'))
 await click(v.button('Publish batch (2)'))
 expect(v.props.onReady).toHaveBeenLastCalledWith(['one.pdf','two.pdf'])
})
it('keeps consented remaining-work publishing clear and excludes a delivered copy from that batch',async()=>{
 const records=['remaining.pdf','delivered.pdf'].map(file=>({file}))
 const v=await mount({files:records,ready:records,allowRemainingIssues:true,fileStates:{'delivered.pdf':{status:'released'}}})
 expect(v.container.textContent).toContain('Includes saved copies with remaining issues, as you chose')
 await click(v.button('Publish batch with remaining issues (1)'))
 expect(v.props.onReady).toHaveBeenCalledWith(['remaining.pdf'])
 expect(api.authorizeReleaseContinuation).not.toHaveBeenCalled()
})
...
it('leaves manual copies outside automatic coverage available', async () => {
 const v = await mount({automaticCoveredFiles:['changes.pdf']})
 expect(v.button('Publish batch (1)').disabled).toBe(false)
 await click(v.button('Publish batch (1)'))
 expect(v.props.onReady).toHaveBeenCalledWith(['ready.pdf'])
})
...
it('blocks manual publishing and additional authorization while automatic status is unavailable, then recovers', async () => {
 const v=await mount({automaticStatusError:'Automatic publication status could not be confirmed.'})
 expect(api.planReleaseContinuation).not.toHaveBeenCalled()
 expect(v.button('Publish batch')).toBeUndefined()
 expect(v.button('Approve eligible changes')).toBeUndefined()
 expect(v.container.textContent).toContain('Automatic publication status is unavailable')
 expect(v.container.textContent).not.toContain('Checking automatic publication…')
 expect(v.props.onReady).not.toHaveBeenCalled()
 expect(api.authorizeReleaseContinuation).not.toHaveBeenCalled()
 await v.render({automaticStatusError:''})
 expect(api.planReleaseContinuation).toHaveBeenCalledOnce()
 await click(v.button('Approve eligible changes'))
 expect(api.authorizeReleaseContinuation).toHaveBeenCalledWith('run-1','fixed-intent')
})
it('discards a stale eligibility response when automatic status becomes unavailable', async () => {
 let resolve
 const deferred=new Promise(r=>{resolve=r})
 const v=await mount({},deferred)
 await v.render({automaticStatusError:'Automatic status unavailable'})
 await act(async()=>resolve(plan))
 expect(v.button('Approve eligible changes')).toBeUndefined()
 expect(api.authorizeReleaseContinuation).not.toHaveBeenCalled()
 api.planReleaseContinuation.mockResolvedValue({...plan,id:'fresh-intent'})
 await v.render({automaticStatusError:''})
 await click(v.button('Approve eligible changes'))
 expect(api.authorizeReleaseContinuation).toHaveBeenCalledWith('run-1','fresh-intent')
})
