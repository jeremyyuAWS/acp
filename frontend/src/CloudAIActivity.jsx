  ...
  return <div className="wd-charts wd-ai-dashboard">
    ...
    <section className="wd-chart wd-model-details" aria-label="Individual model usage">
      ...
      {!models.length ? <p>No recorded model calls for this run.</p> : <div className="wd-model-scroll" tabIndex={0} role="region" aria-label="Scrollable model usage table">
        <table><thead><tr><th>Provider and model</th><th>Attempts</th><th>Awaiting result</th><th>Input tokens</th><th>Output tokens</th><th>Average attempt time</th><th>Median / P95</th><th>Usable output</th><th>Model load / inference</th></tr></thead>
          <tbody>{models.map(model => <tr key={model.id}><th scope="row">{model.label}</th><td>{number(model.value)}</td><td>{number(model.active)}</td><td>{number(model.input_tokens)}</td><td>{number(model.output_tokens)}</td><td>{number(model.average_seconds)}{model.average_seconds != null ? ' s' : ''}<br /><small>{number(model.timed_attempts)} measured attempts</small></td><td>{number(model.median_seconds)} / {number(model.p95_seconds)} s</td><td>{number(model.usable_percent)}{model.usable_percent != null ? '%' : ''}<br /><small>{number(model.validated_attempts)} validated · {number(model.validation_unavailable)} unavailable</small></td><td>{number(model.provider_timing?.model_load_ms?.average)} / {number(model.provider_timing?.inference_ms?.average)} ms<br /><small>{number(model.provider_timing?.model_load_ms?.measured_attempts)} load · {number(model.provider_timing?.inference_ms?.measured_attempts)} inference measurements</small></td></tr>)}</tbody></table>
      </div>}
      <p>Usable output rate includes only settled attempts with known validation. Missing validation and partial coverage are not counted as failures. P95 uses the recorded attempt durations; it is not pure model latency. Model load and inference are provider-reported measurements; missing measurements stay unavailable.</p>
