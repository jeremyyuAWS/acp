...
describe('AssessRunner drives the real analysis when it is deferred (ADR 0020)', () => {
  it('branches on the deferred response and polls the real scan', () => {
    ...
    expect(s).toMatch(/\.then\(startAssessmentRequest\)\.then\(\(resp\)/)   // reads the response, not fire-and-forget
  ...
  it('refreshes the scan Drive token before the deferred fan-out downloads (stale-token gap)', () => {
    ...
    expect(s).toMatch(/refreshScanDriveToken\(runId\)\)\.catch\(\(\) => \{\}\)\.then\(startAssessmentRequest\)/)
  })
  ...
  it('sends the pre-run screen\'s decision, not only its own local toggle', () => {
    ...
    expect(s).toMatch(/opts\?\.fix_approval_policy\s*\n\s*\? assessScan\(runId, opts\?\.level \|\| level, opts \? !!opts\.includeLifecycleFlagged : !ignoreLifecycle, opts\.fix_approval_policy\)/)
