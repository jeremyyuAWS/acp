...
const baseOrigins = [['rules', 'Rules'], ['first_ai', 'First AI usable proposals'], ['fallback_ai', 'Additional fallback proposals']]
const validCount = n => Number.isSafeInteger(n) && n >= 0
const number = n => validCount(n) ? n.toLocaleString() : 'Unavailable'

export default function RemediationContribution({ snapshot, inlineDrilldown = false }) {
  const [selection, setSelection] = useState(null)
  const trigger = useRef(null)
  const inlineDetails = useRef(null)
  const close = useCallback(() => {
    setSelection(null)
    if (inlineDrilldown) trigger.current?.focus()
  }, [inlineDrilldown])
  useEffect(() => { if (inlineDrilldown && selection) inlineDetails.current?.focus() }, [inlineDrilldown, selection])
  ...
  const origins = Object.hasOwn(snapshot?.contributions || {}, 'fallback_2_ai')
    ? [...baseOrigins, ['fallback_2_ai', 'Second fallback additional proposals']] : baseOrigins
  ...
  const select = (field, key, label, event) => {
    trigger.current = event.currentTarget
    setSelection({ field, key, label })
  }
  const findingDetails = (<div className="contribution-findings">
  ...
  return <section className="remediation-contribution" aria-label="Measured contribution">
    ...
    {!available ? <p>Measured contribution is unavailable. This run does not have a complete link to its original findings; missing evidence is not a zero count.</p> : <>
      ...
      <div className="contribution-bars" aria-label="Reconciled outcomes of original findings">
        {outcomes.map(([key, label]) => <div className="contribution-bar-row" key={key}>
          <button type="button" onClick={event => select('state', key, label, event)}>{label}: {number(snapshot.outcomes?.[key])}</button>
      ...
      <div className="contribution-bars" aria-label="Contribution by source, scaled to original findings">
        {origins.map(([key, label]) => <div className="contribution-bar-row" key={key}>
          <button type="button" onClick={event => select('origin', key, label, event)}>{label}: {number(snapshot.contributions?.[key])}</button>
      ...
      <table><caption>Contribution counts and matching findings</caption><thead><tr><th scope="col">Source</th><th scope="col">Original findings</th></tr></thead><tbody>
        {origins.map(([key, label]) => <tr key={key}><th scope="row"><button type="button" onClick={event => select('origin', key, label, event)}>{label}</button></th><td>{number(snapshot.contributions?.[key])}</td></tr>)}
      </tbody></table>
      <table><caption>Current outcomes of original findings</caption><thead><tr><th scope="col">Outcome</th><th scope="col">Original findings</th></tr></thead><tbody>
        {outcomes.map(([key, label]) => <tr key={key}><th scope="row"><button type="button" onClick={event => select('state', key, label, event)}>{label}</button></th><td>{number(snapshot.outcomes?.[key])}</td></tr>)}
      </tbody></table>
    ...
    {available && selection && (inlineDrilldown ? <section ref={inlineDetails} tabIndex={-1} aria-label={`${selection.label} finding details`}>
      <button type="button" onClick={close}>Back to contribution</button>
      <h4>{selection.label}</h4>
      <p>{number(matches.length)} matching original findings · Snapshot {snapshot.snapshot_id || 'unavailable'}</p>
      {findingDetails}
    </section> : <Drawer title={selection.label} subtitle={`${number(matches.length)} matching original findings · Snapshot ${snapshot.snapshot_id || 'unavailable'}`} onClose={close}>
      {findingDetails}
    </Drawer>)}
  </section>
}
