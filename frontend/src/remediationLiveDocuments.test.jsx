...
it('filters recorded file coverage independently of successful remediation and clears it with Show all', async () => {
  ...
  expect(container.querySelectorAll('.progress-queue-file')).toHaveLength(1)
  expect(container.querySelector('.progress-queue-list').textContent).toContain('A.docx')
...
it('filters documents with the shared recorded progress counts without conflating verification and release', async () => {
  ...
  const {container}=await mount({snapshot:{batch_id:'batch'},connected:true})
  await act(async()=>vi.advanceTimersByTime(400))
  expect(container.querySelector('.progress-verified').getAttribute('aria-label')).toBe('Show documents: Verified (1)')
  expect(container.querySelector('.progress-ready').getAttribute('aria-label')).toBe('Show documents: Ready to publish (0)')
...
it('filters assessment fallback by supplied progress and clears filters when assessment changes', async () => {
  ...
  expect(container.querySelectorAll('.progress-queue-file')).toHaveLength(1)
  expect(container.querySelector('.progress-queue-list').textContent).toContain('A.docx')
  expect(container.querySelector('.progress-queue-list').textContent).not.toContain('B.docx')
...
it('overlays recorded Release readiness and current delivery receipt onto live findings',async()=>{
  ...
  await act(async()=>vi.advanceTimersByTime(400))
  expect(container.querySelector('.progress-published').getAttribute('aria-label')).toBe('Show documents: Published (1)')
  expect(container.querySelector('.progress-ready').getAttribute('aria-label')).toBe('Show documents: Ready to publish (1)')
  expect(container.querySelector('.progress-processing').getAttribute('aria-label')).toBe('Show documents: Processing (0)')
})
...
it('counts assessed attention and unopened documents without a legacy finding ledger', async () => {
  ...
  await act(async()=>vi.advanceTimersByTime(400))
  expect(container.querySelector('.progress-attention').getAttribute('aria-label')).toBe('Show documents: Needs attention (32)')
  expect(container.querySelector('.progress-verified').getAttribute('aria-label')).toBe('Show documents: Verified (0)')
  ...
  expect(container.querySelector('.progress-queue-count').textContent).toBe('32 of 32 files shown')
})
...
it('shows a leased document processing while its finding ledger is unavailable',async()=>{
  ...
  expect(container.querySelector('.progress-attention').getAttribute('aria-label')).toBe('Show documents: Needs attention (1)')
...
it('moves the existing five document KPIs into the matching workflow card and retains filtering', async () => {
  ...
  expect(host.querySelectorAll('.remediation-progress-summary-counts:not(.file-coverage-counts) button')).toHaveLength(5)
  expect(host.querySelector('.progress-published').getAttribute('aria-label')).toBe('Show documents: Published (1)')
...
it('keeps the same live table through polling gaps and animates recorded increases with deltas', async () => {
  vi.useFakeTimers()
  const snapshot={batch_id:'batch'}
  getFindingDispositions.mockResolvedValue({available:true,batch_id:'batch',items:files.map((f,i)=>({finding_id:`f${i}`,file:f.file,rule_id:'1.1.1',disposition:'awaiting_review'}))})
  listHitlQueue.mockResolvedValue([]);getScanRemediationDiffs.mockResolvedValue({items:[],total:0})
  const {root,container}=await mount({snapshot,connected:false})
  const table=container.querySelector('.live-document-table')
  expect(table).not.toBeNull()
  await act(async()=>vi.advanceTimersByTime(400))
  getFindingDispositions.mockResolvedValue({available:true,batch_id:'batch',items:files.map((f,i)=>({finding_id:`f${i}`,file:f.file,rule_id:'1.1.1',disposition:i ? 'awaiting_review' : 'resolved_verified'}))})
  await act(async()=>root.render(<RemediationLiveDocuments {...props} snapshot={{...snapshot,fixes:{verified:1}}} connected={false} />))
  await act(async()=>vi.advanceTimersByTime(400))
  expect(container.querySelector('.live-document-table')).toBe(table)
  expect(container.querySelector('.progress-verified .livecounter-delta').textContent).toBe('+1')
  getFindingDispositions.mockResolvedValue({available:false})
  await act(async()=>root.render(<RemediationLiveDocuments {...props} snapshot={{...snapshot,fixes:{verified:2}}} connected={false} />))
  await act(async()=>vi.advanceTimersByTime(400))
  expect(container.querySelector('.live-document-table')).toBe(table)
  expect(container.querySelector('.progress-verified').getAttribute('aria-label')).toContain('(1)')
  expect(container.textContent).toContain('last confirmed counts remain visible')
})
it('renders one flat clickable outcome pill and preserves selection while filtering documents', async () => {
  vi.useFakeTimers()
  getFindingDispositions.mockResolvedValue({available:true,batch_id:'batch',items:files.map((file,index)=>({finding_id:`flat-${index}`,file:file.file,rule_id:'1.1.1',disposition:index?'awaiting_review':'resolved_verified'}))})
  listHitlQueue.mockResolvedValue([])
  getScanRemediationDiffs.mockResolvedValue({items:[],total:0})
  const {container}=await mount({snapshot:{batch_id:'batch'}})
  await act(async()=>vi.advanceTimersByTime(401))
  const filters=container.querySelector('[aria-label="Live finding outcomes"]')
  const all=filters.querySelector('.live-finding-filter--all')
  const verified=filters.querySelector('[aria-label="Verified: 1 findings"]')
  expect(all.getAttribute('aria-pressed')).toBe('true')
  expect(verified.querySelector('.remediation-category-pill')).toBeNull()
  expect(verified.querySelector('strong').textContent).toBe('1')
  expect(verified.title).toContain('Recorded verification checks')
  await act(async()=>verified.click())
  expect(verified.getAttribute('aria-pressed')).toBe('true')
...
it('keeps authoritative unreadable assessment files visible as Blocked without invented counts or fix actions', async () => {
  vi.useFakeTimers()
  getFindingDispositions.mockResolvedValue({available:true,batch_id:'batch',items:[]})
  const snapshot = {batch_id:'batch',assessment_blocked_files:[{file:'broken.pdf',reason:'PDF parser could not read the saved source.',category:'unreadable'}]}
  const {container} = await mount({files:[],snapshot})
  await act(async () => vi.advanceTimersByTime(400))
  const row = container.querySelector('tbody tr')
  expect(row.textContent).toContain('broken.pdf')
  expect(row.textContent).toContain('Blocked')
  expect(row.textContent).toContain('PDF parser could not read the saved source.')
  expect(row.querySelector('.col-findings').textContent).toBe('—')
  expect(row.querySelector('.col-criteria').textContent).toBe('—')
  expect(row.textContent).not.toContain('Reconciling')
  expect(row.textContent).not.toContain('Awaiting recorded outcomes')
  expect(row.querySelector('button')).toBeNull()
})
it('does not label a ledger mismatch as an unreadable assessment', async () => {
  vi.useFakeTimers()
  getFindingDispositions.mockResolvedValue({available:true,batch_id:'batch',items:[]})
  const {container} = await mount({snapshot:{batch_id:'batch'}})
  await act(async () => vi.advanceTimersByTime(400))
  expect(container.querySelector('.assessment-blocked-document')).toBeNull()
  expect(container.querySelector('tbody').textContent).toContain('Reconciling')
})
it('closes a stale fixes view when an authoritative assessment blocker arrives', async () => {
  getFileRemediationDiffs.mockResolvedValue([fix])
  const {root,container} = await mount()
  await act(async () => [...container.querySelectorAll('button')].find(button => button.textContent.includes('View fixes')).click())
  expect(container.querySelector('.remediation-file-page')).not.toBeNull()
