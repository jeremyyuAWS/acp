...
export default function RemediationProgressSummary({ documents = [], selected, onSelect, reconciling = false, animate = false, coverage, onCoverageSelect, selectedCoverage, baselineDocumentCounts, startedAt, queueMode = false, variant, compact = false }) {
  ...
  if (compact) return <section className="remediation-progress-summary" aria-label="File processing coverage">
    ...
    {onSelect && <button type="button" className="ghost small" aria-haspopup="dialog" onClick={() => onSelect(null)}>Show all {documents.length} document{documents.length === 1 ? '' : 's'}</button>}
    {reconciling && <p role="status">Refreshing confirmed processing results…</p>}
  </section>
