...
it('reconciles screenshot populations without counting status checks as human input',()=>{
 ...
 expect(result.humanTotal).toBe(190)
 expect(result.humanTotal).toBe(remediationReviewCounts(rows,{}, {},true).pendingItems)
 expect(result.statusTotal).toBe(112)
 expect(result.humanTotal+result.statusTotal).toBe(302)
 expect(result.notices.filter(n=>n.population==='human').reduce((s,n)=>s+n.count,0)).toBe(190)
 expect(result.notices.find(n=>n.key==='status:missing-proposals').count).toBe(112)
 expect(result.notices.find(n=>n.key==='missing-proposals').count).toBe(10)
})
...
it('renders the live panel with separate reconciled groups under automatic approval',async()=>{
 ...
 expect(summary.textContent).toContain('190 need your input')
