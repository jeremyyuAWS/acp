...
export default function GraphFindingsDrawer({ file, scanId, onClose, items = [], readOnly = false, onAct }) {
  ...
  const attention = items.map(item => ({ ...item, kind: 'review', title: item.rule_name || sc(item.rule_id), detail: item.reason || item.description || 'Review the recorded proposal or remaining work.', page: item.page, locator: item.locator || item.evidence?.[0]?.locator }))
  ...
  return <Drawer title={file.file} subtitle="Findings & changes" onClose={onClose}>
    <div className="graph-findings-drawer">
      ...
      <section className="graph-findings-preview" aria-label="Finding preview">
      ...
      <div className="graph-findings-tabs" role="tablist" aria-label="Findings and changes">
        {[['attention', 'Needs your attention'], ['changes', 'Changes made'], ['all', 'All findings']].map(([key, label]) => <button type="button" key={key} role="tab" tabIndex={activeTab === key ? 0 : -1} aria-selected={activeTab === key} onKeyDown={event => { const keys = ['attention', 'changes', 'all']; const index = keys.indexOf(key); const next = event.key === 'ArrowRight' ? (index + 1) % 3 : event.key === 'ArrowLeft' ? (index + 2) % 3 : event.key === 'Home' ? 0 : event.key === 'End' ? 2 : null; if (next === null) return; event.preventDefault(); setTab(keys[next]); setSelected(null); setReview(null); event.currentTarget.parentElement.querySelectorAll('[role="tab"]')[next].focus() }} aria-controls="graph-findings-panel" onClick={() => { setTab(key); setSelected(null); setReview(null) }}>{label}</button>)}
      </div>
      <section id="graph-findings-panel" role="tabpanel" aria-label={activeTab === 'attention' ? 'Needs your attention' : activeTab === 'changes' ? 'Changes made' : 'All findings'}>
        ...
        {rows.map((row, index) => <article className={`graph-finding graph-finding--${row.kind}`} key={`${row.id || row.rule_id || row.wcag}-${index}`}>
          <button type="button" className="graph-finding-select" aria-pressed={selected?.kind === row.kind && selected?.title === row.title && selected?.seq === row.seq && selected?.id === row.id && selected?.page === row.page && selected?.locator === row.locator} onClick={() => setSelected(row)}><strong>{row.title || 'Finding'}</strong><span>{row.page ? `Page ${row.page}` : row.locator || 'Location unavailable'}</span></button>
          ...
          {row.kind === 'review' && <button type="button" className="ghost small" onClick={() => setReview(row)}>{requiresPdfSourceEditing(row) ? 'View editing guidance' : 'Review recorded suggestion'}</button>}
        </article>)}
        {review && <><button type="button" className="ghost small" onClick={() => setReview(null)}>Close suggestion</button>{requiresPdfSourceEditing(review) ? <div className="graph-finding"><strong>Edit the source document or use a PDF accessibility editor.</strong><p>This outline is guidance; ACP cannot create the missing PDF structure tree automatically.</p><p>{text(review.suggested_value || review.proposed_value)}</p></div> : readOnly ? <div className="graph-finding"><strong>Historical record · viewing only</strong><p>{text(review.suggested_value || review.proposed_value)}</p></div> : <EvidenceCard key={review.id} item={review} onAct={onAct} onResolved={() => setReview(null)} />}</>}
      </section>
