...
export default function RemediationImpactCard({ runId, onRun, runBusy = false, myEmail = '', readOnly = false, refreshKey = 0, scopeFiles, assessmentTotal, renderAssessment, releaseOption, requireAnswers = false, releaseAnswered = true, automaticRelease = false }) {
  ...
  const readinessText = {
    ...
    local_endpoint_access_denied: 'The private AI endpoint rejected access. Ask an administrator to check its access credentials, then retry readiness, or choose Rules only. Changing the cloud spending limit will not fix local endpoint access.',
