  ...
  for (const event of latest.values()) {
    ...
    if (event.kind === 'remediate.vision_retry_blocked') {
      ...
      if (event.reasonCode === 'vision_spending_reconciliation_required') {
        ...
        ? { key: event.key, label: 'Local AI setup needs attention', responsibility: 'The saved plan permits local AI only. Check the private endpoint and configured models in Settings, then retry the failed generation. This earlier record does not identify the exact local failure; increasing a cloud spending limit will not fix a local-only plan.', tone: 'review' }
