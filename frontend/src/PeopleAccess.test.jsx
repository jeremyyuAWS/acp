...
describe('People access onboarding', () => {
  ...
  it('uses one dialog for either provider and returns focus when cancelled', async () => {
    ...
    const microsoft = document.querySelector('input[value="microsoft"]')
    await act(async () => microsoft.click())
    expect(document.body.textContent).toContain('Microsoft guest invitations are not connected')
  ...
  it('submits the chosen identity provider and honest role', async () => {
    ...
    await act(async () => [...document.querySelectorAll('button')].find((b) => b.textContent === 'Add person').click())
    await settle()
