...
export default function LiveThroughput({ points = [], ratePerMin = null, label = 'Throughput', unitLabel = 'completed', compact = false, mini = false }) {
  ...
  if (mini && points.length < 2) return null
  if (points.length < 2) return (
    <div className="muted" style={{ fontSize: 12.5 }}>{label} · calibrating…</div>
  )
  const width = mini ? 92 : compact ? 220 : 260, height = mini ? 20 : compact ? 42 : 72
  const plot = compact || mini
  ...
  const chart = (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} role="img"
      ...
      <polyline points={coords} fill="none" stroke="var(--purple,#6f4a78)" strokeWidth={mini ? 1.75 : 2.25}
                strokeLinecap="round" strokeLinejoin="round" />
      {!compact && !mini && <text x={plot.left + 4} y={plot.top + 10} fontSize="8.5" fill="var(--muted,#667085)">bars: movement/update</text>}
    </svg>
  )
