...
export default function AcceptedRemediationPlanSummary({ policy, authorization, loading = false }) {
  ...
  const tools = policy.ai === 0 ? 'Rules only' : usesAI
    ? policy.ai_zone === 'local' ? 'Local models' : policy.ai_zone === 'any' ? 'Local + cloud models' : 'Rules + configured AI'
    : 'Not recorded'
  ...
  return <section className="accepted-remediation-plan" aria-label="Accepted remediation plan">
    ...
    <dl>
      <div><dt>Rule-based fixes</dt><dd>{policy.fix_approval_policy?.mode === 'review' ? 'Review or edit the document before applying protected repairs' : ruleChoices[policy.rule_based] || 'Not recorded'}</dd></div>
      {policy.fix_approval_policy?.mode === 'custom' && <div><dt>Criteria requiring review</dt><dd>{policy.fix_approval_policy.review_scs.join(', ') || 'None'}</dd></div>}
      <div><dt>Tools</dt><dd>{tools}</dd></div>
      {usesAI && <>
        <div><dt>AI suggestions</dt><dd>{policy.fix_approval_policy ? savedApprovalLabel(policy) : policy.auto_approve_ai === true ? 'Apply supported suggestions automatically' : 'Review suggestions before applying'}</dd></div>
        ...
        {policy.document_wide_ai === true && <div><dt>Document input</dt><dd>{policy.cloud_input_strategy === 'automatic' ? 'Selected automatically for each document' : policy.document_wide_input_mode === 'native_pdf' ? 'Full PDF · advanced preview; other formats use document context' : 'Document context · extracted text and supported images'}</dd></div>}
      </>}
