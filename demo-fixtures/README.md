# Accessibility demo fixtures — Word + Excel

Two deliberately-broken business documents that seed **one instance of every in-scope
WCAG failure ACP can action** on that format. Scanning them, then running remediation,
walks a live audience through all three remediation lanes and drives the file from many
findings to a clean certificate — the proof behind "100% actioned on Word and Excel."

| File | In-scope criteria exercised |
|------|-----------------------------|
