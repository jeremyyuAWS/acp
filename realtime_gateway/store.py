...
from api.realtime_events import MAX_REPLAY_EVENTS, RealtimeEvent, parse_last_event_id, stream_key
...
class RedisEventStore:
    """Read the canonical owner stream without touching any legacy queue or SSE keys."""
    ...
    async def listen(self, scope: str, cursor: str, *, block_ms: int) -> AsyncIterator[tuple[str, RealtimeEvent]]:
        current = cursor
        while True:
            rows = await self.redis.xread({stream_key(scope): current}, count=MAX_REPLAY_EVENTS, block=block_ms)
